...
# Confirm network access to OpenAI
try:
...
# GPT prompt logic (simplified and improved)
prompt_template = """
...
# Process each article and extract relevant info
results = []
...
for article in articles:
    ...
    try:
        ...
        if not reply.startswith("SKIP"):
            ...
            if not actors_str:
                actors_str = "UNKNOWN ACTORS"

            actors_list = [actor.strip() for actor in actors_str.split(",")]

            # Define Tier A and Tier B actors
            ...
            # Filter out Tier C actors and ensure only Tier A or Tier B actors
            ...
            if len(a_tier_actors) > 2:
                ...
                filtered_actors = a_tier_actors[:2]

            # Handle project title and summary
            title_str = article["title"].strip().upper()
